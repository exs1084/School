...
# ##### BEGIN STUDENT CODE BLOCK 1 #####
	move	$s0, $a0	#copy args into s reg
	move	$s1, $a1
	jal	find_gcd	#find gcd
	move	$s2, $v0	#store gcd
	move	$t9, $zero
	div	$s0, $s0, $s2
	div	$s1, $s1, $s2	#divide by gcd
simp_loop:
	slt	$t1, $s0, $s1	#if num<denom, t1=1
	bne	$t1, $zero, simp_ret
	addi	$t9, $t9, 1	#increment integer
	sub	$s0, $s0, $s1	#reduce numer
	j 	simp_loop
simp_ret:
	move	$v0, $s0
	move	$v1, $s1
...
while_gcd:
	beq	$a0, $a1, gcd_ret	#while num1!=num2
	sub	$t1, $a0, $a1		#D=num1-num2
	slt	$t2, $t1, $zero		#is D<0?
	beq	$t2, $zero, D_pos	#if no, continue
	sub	$t1, $zero, $t1		#if yes, change that
D_pos:
	slt	$t2, $a1, $a0		#num2<num1?
	beq	$t2, $zero, set_num2	#if not change num2
	move	$a0, $t1		#num1=D
	j	while_gcd
set_num2:
	move	$a1, $t1		#num2=D

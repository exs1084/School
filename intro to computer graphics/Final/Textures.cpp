...
GLfloat lightColor2[4] = { 0.9, 1.0, 0.9, 1.0 };
GLfloat lightPos2[4] = { 0.0, 5.0, 2.0, 1.0 };

GLfloat ambCol2[4] = { 0.7, 0.7, 0.7, 1.0 };

GLuint locs2[10];

void setTexProperties(GLfloat Ka, GLfloat Kd, GLfloat Ks, GLfloat specExponent, 
	GLfloat light2Col[4], GLfloat light2Pos[4], GLfloat ambientCol[4]){
	for(int i=0;i<4;i++){
		lightColor2[i] = light2Col[i], lightPos2[i]=light2Pos[i];
		ambCol2[i] = ambientCol[i];
	}
	kA2=Ka, kD2=Kd, kS2=Ks, specEx2 = specExponent;
}
...
{
    // Add your code here.
    smiley = SOIL_load_OGL_texture("wall.png", SOIL_LOAD_AUTO, 
	...
	frowny = SOIL_load_OGL_texture("table.jpg", SOIL_LOAD_AUTO, 
    SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | 
    SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT);

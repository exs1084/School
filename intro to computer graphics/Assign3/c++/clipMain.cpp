...
void makePolygonOutlines( Canvas &C ) {
    ...
    C.setColor( 1.0f, 1.0f, 0.0f );    // yellow
    drawPolygon( quad5, quad5_nv, C );
    C.setColor( 0.6f, 0.196f, 0.8f );  // dark orchid
...
void makePolygons( Canvas &C ) {
    ...
    C.setColor( 0.0f, 1.0f, 0.0f ); // green
    nv[1] = clipper.clipPolygon( 4, quad2, tmp, clip1[0], clip1[1] );
    ...
    C.setColor( 0.0f, 0.0f, 1.0f );	// blue
    nv[2] = clipper.clipPolygon( 4, quad3, tmp, clip2[0], clip2[1] );
    if( nv[2] > 0 ) {
    ...
    C.setColor( 1.0f, 0.0f, 1.0f );	// magenta
    nv[3] = clipper.clipPolygon( 4, quad4, tmp, clip1[0], clip1[1] );
    if( nv[3] > 0 ) {
    ...
    C.setColor( 1.0f, 0.5f, 1.0f ); // red-greenish-blue
    nv[4] = clipper.clipPolygon( 5, pent1, tmp, clip3[0], clip3[1] );
    if( nv[4] > 0 ) {
    ...
    C.setColor( 0.7f, 0.7f, 0.7f );	// gray
    nv[5] = clipper.clipPolygon( 7, hept1, tmp, clip4[0], clip4[1] );
    if( nv[5] > 0 ) {
    ...
    C.setColor( 0.871f, 0.722f, 0.529f ); // burlywood
    nv[6] = clipper.clipPolygon( nona1_nv, nona1, tmp, clip6[0], clip6[1] );
    if( nv[6] > 0 ) {
    ...
    C.setColor( 1.0f, 0.64705f, 0.0f );  // orange
    nv[7] = clipper.clipPolygon( deca1_nv, deca1, tmp, clip5[0], clip5[1] );
    if( nv[7] > 0 ) {
    ...
    C.setColor( 0.6f, 0.196f, 0.8f );    // dark orchid
    nv[9] = clipper.clipPolygon( tri1_nv, tri1, tmp, clip2[0], clip2[1] );
    if( nv[9] > 0 ) {
